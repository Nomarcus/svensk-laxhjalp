...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({ model: IMAGE_MODEL, contents: { parts: [{ text: 'Skapa en pedagogisk illustration: ' + prompt }] }, config: { responseModalities: ['image', 'text'], imageConfig: { aspectRatio: '1:1' } } });
    if (response.candidates?.[0]?.content?.parts) { for (const part of response.candidates[0].content.parts) { if (part.inlineData) return res.json({ imageData: 'data:image/png;base64,' + part.inlineData.data }); } }
    res.json({ imageData: null });
  } catch (error: any) { console.error('Image error:', error.message); res.status(500).json({ error: 'Bildgenerering misslyckades.' }); }
}
