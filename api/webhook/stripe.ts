...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (event.type === 'checkout.session.completed') { const s = event.data.object as Stripe.Checkout.Session; const uid = s.metadata?.firebaseUID; if (uid && s.subscription) { const sub = await stripe.subscriptions.retrieve(s.subscription as string); await db.doc('users/' + uid).set({ tier: 'pro', subscriptionStatus: 'active', stripeSubscriptionId: sub.id, stripeCustomerId: s.customer as string, currentPeriodEnd: new Date(sub.current_period_end * 1000).toISOString(), cancelAtPeriodEnd: sub.cancel_at_period_end }, { merge: true }); } }
