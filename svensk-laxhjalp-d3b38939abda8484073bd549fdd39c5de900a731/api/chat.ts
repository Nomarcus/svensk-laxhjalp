...
export default async function handler(req: any, res: any) {
  ...
  // Subscription check
  const actionType = req.body?.imageBase64 ? 'imageAnalysis' : 'chat';
  const check = await checkSubscription(uid, actionType);
  if (!check.allowed) {
    return res.status(check.statusCode).json({ error: check.error, upgradeRequired: true });
  }
