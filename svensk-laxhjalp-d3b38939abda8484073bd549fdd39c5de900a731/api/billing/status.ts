...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const usage = (await db.doc('users/' + decoded.uid + '/usage/' + today).get()).data() || {};
    const creditsTotal: number = data.monthlyCreditsTotal ?? 0;
    const creditsUsed: number = data.monthlyCreditsUsed ?? 0;
    res.json({
      tier: data.tier || 'none',
      ...
      usage: {
        ...
        illustrationCount: usage.illustrationCount || 0,
      },
    });
