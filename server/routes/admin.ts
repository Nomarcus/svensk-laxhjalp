...
/**
 * Sole admin: either (1) ADMIN_UID matches token uid — optional ADMIN_EMAIL must also match, or
 * (2) no ADMIN_UID but ADMIN_EMAIL matches token email (single account only).
 */
function isSoleAdmin(req: AuthenticatedRequest): boolean {
  ...
  if (adminUid) {
    if (!req.uid || req.uid !== adminUid) return false;
    if (adminEmail && (!req.email || req.email.toLowerCase() !== adminEmail)) return false;
    return true;
  }

  if (adminEmail) {
    return Boolean(req.email && req.email.toLowerCase() === adminEmail);
  }
...
/** Sum usage docs for a calendar day (document id YYYY-MM-DD under each user's usage subcollection). */
async function aggregateUsageForDate(db: Firestore, dateStr: string) {
  ...
  snap.forEach((doc) => {
    ...
    const d = doc.data();
    const c = typeof d.chatCount === 'number' ? d.chatCount : 0;
    const im = typeof d.imageCount === 'number' ? d.imageCount : 0;
    aiChats += c;
    imageAnalyses += im;
    if (c > 0 || im > 0) {
      perUser.push({ uid, aiChats: c, imageAnalyses: im });
    }
  });
    ...
    res.json({
      ...
      config: {
        soleAdminConfigured: Boolean(
          process.env.ADMIN_UID?.trim() || process.env.ADMIN_EMAIL?.trim(),
        ),
      ...
      engagement: {
        withEmail,
        guests,
        activeLast7DaysInSample: activeLast7d,
      },
      subscription: {
        byTier: tierBuckets,
