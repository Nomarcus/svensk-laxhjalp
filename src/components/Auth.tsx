...
import { signInWithGoogle, signInWithFacebook, signInWithMicrosoft, signInWithEmail, signUpWithEmail, resetPassword, signInAsGuest } from '../firebase';
...
export default function Auth({ onShowPrivacy, onShowTerms }: AuthProps) {
  ...
  if (view === 'reset-password') {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#F7F7F2] via-[#F5F5F0] to-[#EFF7F2] flex items-start justify-center pt-[12vh] md:pt-[15vh] p-4 font-sans">
        <div className="max-w-md w-full bg-white rounded-[32px] p-8 md:p-12 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.32)] border border-black/[0.06] relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-2 bg-emerald-600" />
          {backButton}
          <div className="mt-4 text-center mb-8">
            <h2 className="text-2xl font-serif italic text-stone-900 mb-2">{t('auth.resetPassword')}</h2>
            <p className="text-stone-500 text-sm">{t('auth.resetDescription')}</p>
          </div>
          {errorBox}
          {resetSent ? (
            <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm rounded-xl px-4 py-3 text-center" dangerouslySetInnerHTML={{ __html: t('auth.resetSent', { email }) }} />
          ) : (
            <form onSubmit={handleResetPassword} className="space-y-4">
              <input type="email" placeholder={t('auth.email')} value={email} onChange={e => setEmail(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-stone-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all text-stone-700" />
      ...
      <div className="min-h-screen bg-gradient-to-b from-[#F7F7F2] via-[#F5F5F0] to-[#EFF7F2] flex items-start justify-center pt-[12vh] md:pt-[15vh] p-4 font-sans">
        <div className="max-w-md w-full bg-white rounded-[32px] p-8 md:p-12 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.32)] border border-black/[0.06] relative overflow-hidden">
          ...
          <form onSubmit={isSignup ? handleEmailSignup : handleEmailLogin} className="space-y-4">
            {isSignup && (
              <input type="text" placeholder={t('auth.name')} value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-stone-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all text-stone-700" />
            )}
            <input type="email" placeholder={t('auth.email')} value={email} onChange={e => setEmail(e.target.value)} required className="w-full px-4 py-3 rounded-xl border border-stone-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none transition-all text-stone-700" />
            <div className="relative">
          ...
          <div className="mt-6 text-center text-sm text-stone-500">
            {isSignup ? (
              <p>{t('auth.hasAccount')}{' '}<button onClick={() => goTo('email-login')} className="text-emerald-600 hover:underline font-medium">{t('auth.login')}</button></p>
            ) : (
              <>
                <p>{t('auth.noAccount')}{' '}<button onClick={() => goTo('email-signup')} className="text-emerald-600 hover:underline font-medium">{t('auth.createAccount')}</button></p>
                <button onClick={() => goTo('reset-password')} className="mt-2 text-stone-400 hover:text-emerald-600 hover:underline text-xs">{t('auth.forgotPassword')}</button>
              </>
            )}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#F7F7F2] via-[#F5F5F0] to-[#ECF6F1] font-sans relative overflow-hidden">
        ...
        <div className="flex-1 lg:w-1/2 flex items-center justify-center p-6 md:p-10 lg:p-14 xl:p-20 bg-gradient-to-br from-white/70 via-white/40 to-emerald-50/40">
          <div className="w-full max-w-md bg-white/95 backdrop-blur-sm rounded-[32px] p-10 shadow-[0_24px_70px_-30px_rgba(16,24,40,0.45)] border border-black/[0.06] text-center relative overflow-hidden">
            ...
            <div className="space-y-3">
              <button onClick={handleGoogleLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white border border-stone-200 py-4 px-6 rounded-2xl font-medium text-stone-700 hover:bg-stone-50 hover:border-stone-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200 transition-all shadow-sm active:scale-[0.98] disabled:opacity-50">
              ...
              <button onClick={handleFacebookLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white border border-stone-200 py-4 px-6 rounded-2xl font-medium text-stone-700 hover:bg-stone-50 hover:border-stone-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200 transition-all shadow-sm active:scale-[0.98] disabled:opacity-50">
                <span className="w-5 h-5 rounded-full bg-[#1877F2] text-white text-[12px] font-bold flex items-center justify-center">f</span>
                {t('auth.loginFacebook')}
              </button>
              <button onClick={handleMicrosoftLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white border border-stone-200 py-4 px-6 rounded-2xl font-medium text-stone-700 hover:bg-stone-50 hover:border-stone-300 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-200 transition-all shadow-sm active:scale-[0.98] disabled:opacity-50">
                <span className="w-5 h-5 grid grid-cols-2 gap-[2px]">
            ...
            <div className="mt-8 pt-6 border-t border-black/5 grid grid-cols-2 gap-4">
              <div className="text-left">
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-1">{t('landing.pedagogical')}</p>
                <p className="text-xs text-stone-500">{t('landing.pedagogicalDesc')}</p>
              </div>
              <div className="text-left">
                <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mb-1">{t('landing.swedishSchool')}</p>
                <p className="text-xs text-stone-500">{t('landing.swedishSchoolDesc')}</p>
              </div>
            </div>
            {(onShowPrivacy || onShowTerms) && (
              <p className="mt-4 text-xs text-stone-400">
                {t('auth.termsConsent')}{' '}
                {onShowTerms && <button onClick={onShowTerms} className="underline hover:text-emerald-600 transition-colors">{t('auth.termsOfService')}</button>}
                {onShowTerms && onShowPrivacy && ` ${t('auth.and')} `}
                {onShowPrivacy && <button onClick={onShowPrivacy} className="underline hover:text-emerald-600 transition-colors">{t('auth.privacyPolicy')}</button>}.
              </p>
            )}
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 pb-20 space-y-20">
        ...
        {/* App slideshow */}
        <section>
          <h2 className="text-3xl font-serif italic text-stone-900 text-center mb-8">{t('landing.seeHowItWorks')}</h2>
          <AppSlideshow />
        </section>

        {/* How it works */}
        <section>
          <h2 className="text-3xl font-serif italic text-stone-900 text-center mb-3">{t('landing.howWeHelp')}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="bg-white rounded-3xl p-8 shadow-sm border border-black/5 text-center">
                ...
                <div className="text-sm font-bold text-emerald-600 mb-2">{t('landing.step')} {s.step}</div>
        ...
        <section>
          <h2 className="text-3xl font-serif italic text-stone-900 text-center mb-3">{t('landing.whyParentsNeedHelp')}</h2>
          <p className="text-center text-stone-500 mb-10 max-w-2xl mx-auto">
        ...
        <footer className="text-center text-stone-400 text-sm pt-4 pb-8 space-y-2">
          ...
          <div className="flex items-center justify-center gap-4">
            {onShowPrivacy && (
              <button onClick={onShowPrivacy} className="underline hover:text-emerald-600 transition-colors">{t('auth.privacyPolicy')}</button>
            )}
            {onShowTerms && (
              <button onClick={onShowTerms} className="underline hover:text-emerald-600 transition-colors">{t('auth.termsOfService')}</button>
            )}
          </div>
        </footer>
      </div>
