...
interface ChatInputProps {
  ...
  onImageActionSelect?: (prompt: string, displayText: string) => void;
  coachMode?: boolean;
}
...
export default function ChatInput({ input, setInput, images, setImages, maxImages = 5, loading, onSubmit, onImageActionSelect, coachMode = false }: ChatInputProps) {
  ...
  const handleImageActionClick = (actionId: typeof homeworkImageActions[number]['id']) => {
    const prompt = t(`chat.imageActionPrompts.${actionId}`);
    const displayText = t(`chat.imageActions.${actionId}`);
                ...
                {homeworkImageActions.map((action) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'shrink-0 inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-white px-3 py-2 text-xs font-medium text-emerald-800 transition-colors hover:border-emerald-300 hover:bg-emerald-100 dark:border-emerald-800/60 dark:bg-slate-900 dark:text-emerald-100 dark:hover:bg-emerald-900/40',
