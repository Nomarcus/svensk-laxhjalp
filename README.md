...
4. Subscriptions: Stripe billing has been removed entirely (2026) — the app is fully free for now. Apple requires **StoreKit In-App Purchase** for digital subscriptions consumed inside an app (guideline §3.1.1), so the next step before re-introducing paid plans is a native IAP integration (e.g. StoreKit 2 + a matching Android Play Billing setup), not Stripe.
